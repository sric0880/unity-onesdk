...
public class SDK360 extends SDK {
//    private QihooUserInfo mQihooUserInfo;
    public SDK360(OneSDK oneSDK) { super(oneSDK); }
...
//                mLoginCb.succeed();
                ...
                mLoginCb.failed("Access token is empty.");
            }
        }
    };
...
//    }
    ...
    @Override
    public void login() {
        Matrix.execute(this.mActivity, getLoginIntent(), mLoginCallback);
    }
    ...
    public void logout() {
        ...
        Matrix.execute(this.mActivity, intent, new IDispatcherCallback() {
            @Override
            public void onFinished(String data) {
                mLogoutCb.succeed();
            }
    ...
    protected IDispatcherCallback mPayCallback = new IDispatcherCallback() {
        ...
        public void onFinished(String data) {
            if(TextUtils.isEmpty(data)) {
                mPayCb.failed(mPayOrder, "支付数据为空");
                return;
            }
            ...
            try {
                ...
                switch (errorCode) {
                    case 0:
                        ...
                        return;
                    case 4010201:
                        mPayCb.failed(mPayOrder, "Access token失效，请重新登录再试");
                        return;
                    case 4009911:
                        mPayCb.failed(mPayOrder, "Qt失效，请重新登录再试");
                        return;
                    case 1:
                    case -1:
                    case -2:
                    default:
                        mPayCb.failed(mPayOrder, jsonRes.optString("error_msg"));
                        return;
                }
            ...
            if (!isCallbackParseOk) {
                mPayCb.failed(mPayOrder, "支付数据格式异常");
            }
        }
    };
        ...
        try {
            if (data != null && data.length() > 0) {
                ...
                if (null != json) {
                    ...
                    qihooPay.setAppName(this.mActivity.getString(R.string.app_name));
                    ...
                    // 启动接口
                    Matrix.invokeActivity(this.mActivity, intent, mPayCallback);
