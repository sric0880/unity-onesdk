...
public class SDKHuawei implements SDK {
...
//    }
    ...
    {
        GameServiceSDK.init(this.activity, APP_ID, PAY_ID, "com.huawei.gb.huawei.installnewtype.provider", new GameEventHandler(){
    ...
    {
        GameServiceSDK.checkUpdate(this.activity, new GameEventHandler(){
    ...
    public void Login() {
        if (initOk)
        {
            GameServiceSDK.login(this.activity, new GameEventHandler(){

                @Override
                public void onResult(Result result) {
                    if (result.rtnCode != Result.RESULT_OK)
                    {
                        Log.i("Unity", "huawei login failed:" + result.toString());
                        UnityPlayer.UnitySendMessage("LoginMng", "OnSdkLogin", "failed|"+result.toString());
                    }else {
                ...
                if ("0".equals(payResp.get(PayParameters.returnCode))) {
                    if ("success".equals(payResp.get(PayParameters.errMsg))) {
                        ...
                        if (s) {
                            ...
                            UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|支付验证失败");
                        }
                    }

                } else if ("30002".equals(payResp.get(PayParameters.returnCode))) {
                    UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|支付超时");
                }
            }
        };
...
//        String sign = Rsa.sign(noSign, PAY_RSA_PRIVATE);
        ...
        */
        ...
        GameServiceSDK.startPay(this.activity, payInfo, handler);
    }
...
//                    Toast.makeText(UnityPlayer.currentActivity, data, Toast.LENGTH_SHORT).show();
                ...
                if (null != json) {
                    ...
                    if (!priceChceckRet)
                    {
                        UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|价格异常");
                        return "";
                    }

                    if ("".equals(productName))
                    {
                        UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|商品名称为空");
                        return "";
                    }
                    // 禁止输入：# " & / ? $ ^ *:) \ < > | , =
                    ...
                    {
                        UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|商品名称异常");
                        return "";
                    }
                    if ("".equals(productDesc))
                    {
                        UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|商品描述为空");
                        return "";
                    }
                    // 禁止输入：# " & / ? $ ^ *:) \ < > | , =
                    ...
                    {
                        UnityPlayer.UnitySendMessage("SDKMng", "OnPay", "fail|商品描述异常");
                        return "";
                    }
                    // 调用公共方法进行支付
                    // call the pay method
